...
$js = "
  ...
  {
  	$.getJSON('wakka.php?wiki=Bazar/json&demand=entries&form=1', function(dataUtilisateurs) {
  ...
  function getLiens()
  {
  	$.getJSON('wakka.php?wiki=Bazar/json&demand=entries&form=2', function(dataLiens) {
      var data = Object.values(dataLiens);
      for (var i=0; i<data.length; i++) {
        tabLiens[i] = new Array();
        var u1 = data[i]['listefiche5part1'];
        tabLiens[i][0]=tabUsers[u1]['bf_titre']+' '+tabUsers[u1]['bf_nom'];
        var u2 = data[i]['listefiche5part2'];
